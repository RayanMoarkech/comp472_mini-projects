...
def main(rush_hours: list[RushHour], heuristic_used: int):
    ...
    for index, rush_hour in enumerate(rush_hours):
        ...
        # Print initial board
        write_search_file(file_name=search_file_name, mode='w', f=1, g=0, h=heuristic,
        ...
        # Sort the list
        open_valid_states.sort(key=take_h)
            ...
            for valid_rush_hour_state in open_valid_states:
                for successor_index, successor in enumerate(successors):
                    ...
                    if is_same:
                        ...
                        successor_h = get_values(valid_rush_hour_state=successor, heuristic_used=heuristic_used)
                        valid_rush_hour_state_h = get_values(valid_rush_hour_state=valid_rush_hour_state,
                                                             heuristic_used=heuristic_used)
                        # compare and pop if successor f is higher
                        if successor_h >= valid_rush_hour_state_h:
            ...
            for visited_board in visited_boards:
                for successor_index, successor in enumerate(successors):
                    ...
                    if is_same:
                        # Get f values of both boards
                        successor_h = get_values(valid_rush_hour_state=successor, heuristic_used=heuristic_used)
                        visited_board_h = get_values(valid_rush_hour_state=visited_board,
                                                     heuristic_used=heuristic_used)
                        # compare and pop if successor f is higher
                        if successor_h >= visited_board_h:
                            successors.pop(successor_index)
...
def take_h1(valid_rush_hour_state):
    return get_h1(rush_hour=valid_rush_hour_state['rushHour'])

# take f value from the valid_rush_hour_state
def take_f_h1(valid_rush_hour_state):
    return get_values(valid_rush_hour_state=valid_rush_hour_state, heuristic_used=1)[0]
